...
main(_) ->
    ...
    try
        benchmark_different_sizes(Env),
        benchmark_prefix_depths(Env),
        benchmark_cursor_pagination(Env)
    after
...
benchmark_different_sizes(Env) ->
    ...
    lists:foreach(fun(Size) ->
        ...
        lists:foreach(fun(I) ->
            ...
            hyper_lmdb:write(Env, Key, <<"x">>)
        end, lists:seq(1, Size)),
        
        % Benchmark list all
        T1 = erlang:monotonic_time(microsecond),
        {ok, _} = hyper_lmdb:list_prefix(Env, <<"">>),
        ListAllTime = erlang:monotonic_time(microsecond) - T1,
        
        % Benchmark list prefix
        T2 = erlang:monotonic_time(microsecond),
        {ok, _} = hyper_lmdb:list_prefix(Env, <<"bench">>),
...
benchmark_prefix_depths(Env) ->
    ...
    lists:foreach(fun({Prefix, Desc}) ->
        ...
        {ok, Keys} = hyper_lmdb:list_prefix(Env, Prefix),
...
benchmark_cursor_pagination(Env) ->
    ...
    lists:foreach(fun(PageSize) ->
        T = erlang:monotonic_time(microsecond),
        Pages = paginate(Env, <<"page">>, PageSize),
...
paginate(Env, Prefix, PageSize) ->
    paginate(Env, Prefix, PageSize, undefined, 0).

paginate(Env, Prefix, PageSize, Cursor, Count) ->
    Opts = case Cursor of
        undefined -> #{limit => PageSize, return_cursor => true};
        _ -> #{limit => PageSize, cursor => Cursor, return_cursor => true}
    end,
    
    case hyper_lmdb:list_prefix(Env, Prefix, Opts) of
        {ok, [], _} -> Count;
        {ok, Keys, NewCursor} when length(Keys) < PageSize -> Count + 1;
        {ok, _, NewCursor} -> paginate(Env, Prefix, PageSize, NewCursor, Count + 1)
    end.